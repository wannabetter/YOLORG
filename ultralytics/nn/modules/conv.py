...
from ultralytics.nn.modules.ddb import DiverseBranchBlock

__all__ = ('Conv', 'LightConv', 'DWConv', 'DWConvTranspose2d', 'ConvTranspose', 'Focus', 'GhostConv',
           'ChannelAttention', 'SpatialAttention', 'CBAM', 'Concat', 'RepConv', 'DConv', 'PConv','DCNv2')
...
class DConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        ...
        self.conv = DiverseBranchBlock(in_channels=c1, out_channels=c2, kernel_size=k, stride=s,
                                       padding=autopad(k, p, d), groups=g, dilation=d, deploy=False)
...
class PartialConv(nn.Module):
    ...
    def __init__(self, dim, n_div=4, forward="split_cat", kernel_size=3) -> None:
        ...
        self.conv = nn.Conv2d(self.dim_conv, self.dim_conv, kernel_size, stride=1, padding=(kernel_size[0] - 1) // 2,
                              bias=False)
...
class PConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        """Initialize Conv layer with given arguments including activation."""
        super().__init__()
        self.conv = PartialConv(dim=c1, kernel_size=k)
        self.bn = nn.BatchNorm2d(c2)
        self.act = self.default_act if act is True else act if isinstance(act, nn.Module) else nn.Identity()
