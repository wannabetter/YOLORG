    ...
    def box_label(self, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        ...
        if self.pil or not is_ascii(label):
            ...
            if label:
                ...
                # 这个注释解开可以用来看物体中心点
                # cv2.circle(self.im,(int((p2[0]-p1[0])/2+p1[0]),int((p1[1]-p2[1])/2+p2[1])),2,(0,255,255),-1)
...
def feature_visualization(x, module_type, stage, n=32, save_dir=Path('runs/detect/exp')):
    ...
    if height > 1 and width > 1:
        ...
        path = save_dir / f"stage{stage}"
        os.mkdir(save_dir / f"stage{stage}")
        ...
        for i in range(n):
            plt.imsave(os.path.join(path,f"channel_{i}.jpg",),blocks[i].squeeze(),dpi=300)
